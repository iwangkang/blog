...
public class GlobalConfigValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (filePageSize < FILE_PAGE_SIZE_RANGE[0]) {
			errors.reject("global.file.toosmall", new Object[] { FILE_PAGE_SIZE_RANGE[0] },
					"文件每页数量不能小于" + FILE_PAGE_SIZE_RANGE[0]);
			return;
		}

		if (filePageSize > FILE_PAGE_SIZE_RANGE[1]) {
			errors.reject("global.file.toobig", new Object[] { FILE_PAGE_SIZE_RANGE[1] },
		...
		if (userWidgetPageSize > USER_FRAGEMENT_PAGE_SIZE_RANGE[1]) {
			errors.reject("global.userWidget.toobig", new Object[] { USER_FRAGEMENT_PAGE_SIZE_RANGE[1] },
		...
		if (tagPageSize < TAG_PAGE_SIZE_RANGE[0]) {
			errors.reject("global.tag.toosmall", new Object[] { TAG_PAGE_SIZE_RANGE[0] },
